...
class GantryFusionMenuTheme extends AbstractRokMenuTheme {
    ...
    public function getFormatter($args){
        ...
        return new GantryFusionMenuFormatter($args);
    }

    public function getLayout($args){
        global $gantry;
        if ($gantry->browser->name == "ie" && $gantry->browser->shortversion=="6"){
            require_once(dirname(__FILE__).'/ie6layout.php');
        }
        else {
            require_once(dirname(__FILE__).'/layout.php');
        }
        return new GantryFusionMenuLayout($args);
    }
}
